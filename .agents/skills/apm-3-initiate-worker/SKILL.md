...
# APM 1.0.2 - Worker Initiation Command
...
All necessary guides and skills are available in `.codex/apm-guides/` and `.agents/skills/` respectively. **Read every referenced document in full - every line, every section.** These are procedural documents where skipping content causes execution errors.
...
- `.codex/apm-guides/task-execution.md` - Task Execution Procedure
- `.codex/apm-guides/task-logging.md` - Task Logging Procedure
...
1. Resolve `(the text provided by the User after the command invocation, if any)` against `.apm/bus/` directory names per `.agents/skills/apm-communication/SKILL.md` §4.2 Agent ID Resolution.
...
1. **Execute:** See `.codex/apm-guides/task-execution.md` §3 Task Execution Procedure. The guide controls validation, execution, and completion.
2. **Log:** Create Task Log per `.codex/apm-guides/task-logging.md` §3 Task Logging Procedure.
3. **Report:** Write Task Report per `.codex/apm-guides/task-logging.md` §3.2 Task Report Delivery.
...
- **Handoff execution:** When User initiates, see `.agents/skills/apm-7-handoff-worker/SKILL.md` for Handoff Log and handoff prompt creation.
